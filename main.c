...
socket_data parse_server_response(const char* server_response, int peer_id) {
    ...
    while (TRUE)
    {
        ...
        memcpy(data.ip, temp_ip_address, temp_length);
        body_pointer = separator_position + 1;
        Print("IP done.");
...
void handle_as_client(int socketfd, user_options options, socket_data media_server_data) {
    ...
    while(TRUE) {
        printf("Connecting to media server...\r\n");
        media_server_socketfd = tcp_socket_connect(media_server_data.ip, media_server_data.port, 10);
...
void handle_as_server(int socketfd, user_options options) {
    ...
    char remote_ip[20];
    int remote_port;
    int client_socketfd = tcp_socket_accept(server_socketfd, remote_ip, &remote_port, 10);
    ...
    while(TRUE) {
        int read_length = tcp_socket_pull(client_socketfd, read_buffer, read_buffer_length, 2);
...
int main(int argc, char* argv[]) {
    ...
    if(options.is_media_server) {
        ...
        printf("Peer ip: %s, port: %d", media_server_data.ip, media_server_data.port);
